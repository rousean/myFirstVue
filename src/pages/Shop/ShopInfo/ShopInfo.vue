  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item" :class="activityColor[support.type]" v-for="(support, index) in info.supports" :key="index">
        ...
        <div class="pic-wrapper">
          <ul class="pic-list" ref="picsUl">
            <li class="pic-item" v-for="(pic, index) in info.pics" :key="index">
              <img width="120" height="90" :src="pic"  alt="图片"/>
            </li>
          </ul>
        </div>
      </section>
      ...
      <section class="section">
        ...
        <ul class="detail">
          <li v-for="(item, index) in [['品类', info.category], ['商家电话', info.phone], ['地址', info.address], ['营业时间', info.workTime]]" :key="index">
            <span class="bold">{{item[0]}}</span>
            <span >{{item[1]}}</span>
          </li>
        </ul>
      </section>
      <div style="text-align: center; font-size: 10px;margin-top: 5px;opacity:0.5">我是有底线的๑ŐεŐ๑</div>
    </div>
  </div>
</template>
